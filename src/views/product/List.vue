  ...
  <page-container>
    ...
    <el-card>
      ...
      >
        ...
        <el-table-column label="选择"  width="100" v-if="props.forArticle">
          <template #default="scope">
            <el-button
                type="primary"
                @click="emit('onChoose',scope.row.id)"
            >选我</el-button>
          </template>
        </el-table-column>
...
const loadList = async () => {
  ...
  const [_err, data] = await productApi.getProducts(listParams).finally(() => {
...
const loadProductCates = async () => {
  const [_err, data] = await productApi.getCategoryTree(1)
  if (_err) {
    return
  }
...
const loadProductTypes = async () => {
  const [_err, data] = await productApi.getProductTypes()
  if (_err) {
    return
  }
...
const handleExportExcel = async () => {
  ...
  try {
    const { jsonToExcel } = await import('@/util/export-to-excel')
    jsonToExcel({
